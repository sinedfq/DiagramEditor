    ...
    public class Mapper {
        ...
        public void FixItem(ref Control res, Point pos, IEnumerable<ILogical> items) {
            foreach (var logic in items) {
                ...
                if (tb != null && new Rect(tb.Value.Clip.TopLeft, new Size()).Sum(item.Bounds).Contains(pos)) res = item;  
        ...
        private DiagramItem[]? GrandImport(string data) {
            ...
            Log.Write("Получен не распознаваемый тип данных :/ Нужен JSON, либо XML, либо YAML");
            return null;
        }
